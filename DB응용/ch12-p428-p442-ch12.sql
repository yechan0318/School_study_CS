...
/*
windows explorer
c drive 선택 - 보기 메뉴 - 숨김항목 - 선택한 항목 숨김 해제 
(경고 창 - 모두 무시) 
ProgrmData 가 보임
programdata - MySQL - MySQL Server 8.0 - my.ini
연결프로그램:메모장 p430 같이 수정
저장시 확장자 ini로

NET STOP MySQL -> NET STOP MySQL80
NET START MySQL -> NET START MySQL80
*/
...
SELECT * FROM FulltextTbl WHERE description LIKE '%남자%' ; -- full test search: 대용량시 overloading
-- p432
CREATE FULLTEXT INDEX idx_description ON FulltextTbl(description); -- 시간 확인
...
SELECT *, MATCH(description) AGAINST('남자* 여자*' IN BOOLEAN MODE) AS 점수 
	FROM FulltextTbl WHERE MATCH(description) AGAINST('남자* 여자*' IN BOOLEAN MODE); -- 매치되는 점수도 출력
